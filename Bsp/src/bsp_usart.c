...
static void UartIRQ(UART_T *_pUart)
{
	...
	if ( ((isrflags & USART_ISR_TXE_TXFNF) != RESET) && (cr1its & USART_CR1_TXEIE_TXFNFIE) != RESET)//USART_CR1_TXEIE_TXFNFIE//USART_CR1_TXEIE
	{
		...
		if (_pUart->usTxCount == 0)
		{
			...
			CLEAR_BIT(_pUart->uart->CR1, USART_CR1_TXEIE_TXFNFIE); //TXEIE -> Transmit data register empty, 0->disable

			/* 使能数据发送完毕中断 */
			//USART_ITConfig(_pUart->uart, USART_IT_TC, ENABLE);
			SET_BIT(_pUart->uart->CR1, USART_CR1_TCIE); //TCIE->Transmission complete interrupt enable 
		}
		else
		{
			_pUart->Sending = 1;
